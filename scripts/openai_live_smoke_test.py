#!/usr/bin/env python
"""Manual, opt-in smoke test: invokes a REAL OpenAI model (ADR-029).

*** This makes a real, billable OpenAI API call. ***

Mirrors `scripts/bedrock_live_smoke_test.py`'s safety pattern exactly. This script is
never run by the automated test suite or CI — it is not a pytest module, and it
additionally refuses to run unless both of the following are true, as an independent
second guard against accidental execution:
...
Requires a real OpenAI API key, either via the OPENAI_API_KEY environment variable (the
`openai` SDK's own default) or --api-key.
"""
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        ...
        "(the openai SDK's own default) if not given.",
    )
    parser.add_argument(
        "--confirm-cost",
        action="store_true",
        help="Required: acknowledges this makes a real, billable OpenAI call",
    )
    parser.add_argument(
        "--hide-response",
        action="store_true",
    ...
    if not args.confirm_cost:
        print(
            "*** This makes a REAL OpenAI invocation and WILL incur cost. ***",
            file=sys.stderr,
        )
    ...
    provider = OpenAIModelProvider(client=client, model_catalogue=catalogue)
    ...
    print(f"Invoking model_alias={args.model_alias!r} ...", file=sys.stderr)
